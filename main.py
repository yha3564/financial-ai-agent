    ...
    def get_dividend_yields(self, tickers):
        ...
        for ticker in tickers:
            try:
                ...
                div_map[ticker] = info.get('dividendYield', 0) or 0
            except:
                div_map[ticker] = 0
        return div_map
...
규칙:
- JSON만 반환
- key_bullish/key_bearish의 assets는 영향받는 보유자산/추적자산 티커
- Nvidia 뉴스 → QQC.TO, QQQM 등 관련 ETF도 포함
- 직접적 영향이 있는 자산만 포함"""
    ...
    def create_rankings(self, asset_impacts):
        ...
        default_hold = self.cost_settings.get('default_holding_months', 3)
        rankings = []
    ...
    def generate_recommendations(self, rankings):
        ...
        has_positive_candidate = any(r['weighted_score'] > 0 and r['magnitude'] > 0
                                     for r in rankings
            ...
            for candidate in buy_list:
                ...
                if price > 0:
                    ...
                    tfsa1_actions.append({
                        ...
                        'expected_pct': candidate['weighted_score'] * 100,
        ...
        if not has_buy:
            ...
            if held and len(held) >= 2:
                ...
                if best_external and best_external['weighted_score'] > strongest['weighted_score']:
                    ...
                    buy_target = strongest if strongest['ticker'] != weakest['ticker'] else None

                if buy_target and buy_target['ticker'] != weakest['ticker']:
                    ...
                    swap_cost = self.calc_swap_cost(w_ticker, buy_target['ticker'], w_value)
                    cost_as_score = swap_cost / w_value if w_value > 0 else 0
                    ...
                    if score_gap > 0:
                        ...
                        if score_gap >= 0.15 or weakest['weighted_score'] < -0.10:
                            ...
                            sell_pct, sell_type = 0.5, 'half'
                        else:
                            sell_pct, sell_type = 0.3, 'partial'

                        sell_shares = round(w_shares * sell_pct, 4)
                        ...
                        tfsa1_actions.append({
                            ...
                            'price': w_price, 'value': sell_value,
                            'score': weakest['weighted_score'],
                            'expected_pct': weakest['magnitude'] * 100,
                            'recommended_price': w_price,
                            'recommended_at': recommended_at
                        })
                        b_price = self.get_price(buy_target['ticker'])
                        if b_price > 0:
                            total_buy = sell_value - actual_cost + available_cash
                            ...
                            tfsa1_actions.append({
                                ...
                                'price': b_price,
                                'value': total_buy,
                                'score': buy_target['weighted_score'],
                                'expected_pct': buy_target['weighted_score'] * 100,
        ...
        for ticker, holding in self.my_holdings_tfsa2.items():
            ...
            if best_alternative:
                ...
                if best_alternative['net_score'] > score + cost_as_score:
                    ...
                    buy_shares = round(actual_buy_value / best_price, 4) if best_price > 0 else 0
                    actions.append({
                        'action': 'SELL', 'type': 'full',
                        'ticker': ticker, 'shares': shares,
                        'price': current_price, 'value': current_value,
    ...
    def check_watch_reminder(self, recommendations):
        ...
        try:
            ...
            today = self.now.strftime('%Y-%m-%d')
            if watch_date == today or not watch_date:
                return None

            old_recs = watch.get('recommendations', {})
            old_tfsa1 = old_recs.get('tfsa1', [])
            old_buys = [a['ticker'] for a in old_tfsa1 if a.get('action') == 'BUY']

            new_tfsa1 = recommendations.get('tfsa1', [])
            new_buys = [a['ticker'] for a in new_tfsa1 if a.get('action') == 'BUY']

            repeated = [t for t in old_buys if t in new_buys]
            if repeated:
    ...
    def format_telegram_report(self, recommendations, afterhours_summary=None):
            ...
            for item in afterhours_summary.get('key_bullish', []):
                ...
                if assets:
                    asset_names = " · ".join(
                        ...
                        if t in self.my_holdings_tfsa1 or t in self.my_holdings_tfsa2 or t in self.all_tracked_assets
                    )
                    if asset_names:
        ...
        report += "💡 오늘 추천\n"
        report += "=" * 37 + "\n"
        report += f"💡 TFSA 1 | 💵 현금: ${existing_cash:.0f} CAD\n"
        ...
        if not sells and not buys:
            ...
            if buys:
                ...
                if sell_total > 0 and existing_cash > 0:
                    report += f"\n💰 매수가능: ${total_available:.0f} CAD (현금 ${existing_cash:.0f} + 매도 ${sell_total:.0f})\n\n"
                elif sell_total > 0:
                    report += f"\n💰 매수가능: ${total_available:.0f} CAD\n\n"
                else:
                    report += f"\n💰 매수가능: ${existing_cash:.0f} CAD\n\n"

                for b in buys:
                    name = self.ticker_names.get(b['ticker'], b['ticker'])
                    is_usd = not b['ticker'].endswith('.TO')
                    fx_note = ""
                    if is_usd:
                        fx_fee = self.cost_settings.get('fx_fee', 0.015)
                        usd_amount = b['value'] * (1 - fx_fee)
                        fx_note = f"  (환전 후 ~${usd_amount:.0f} USD)"
                    report += f"{b['ticker']} ({name})\n{b['shares']}주 @${b['price']:.2f} = ${b['value']:.2f}{fx_note}  ({b['expected_pct']:+.1f}% 예상)\n"
            ...
            for ticker, data in recommendations['tfsa2'].items():
                ...
                for action in actions:
                    if action['action'] == 'SELL':
                        ...
                        report += f"💰 매수가능: ${sell_val:.0f} CAD\n\n"
                        is_usd = not action['ticker'].endswith('.TO')
                        fx_note = ""
                        if is_usd:
                            fx_fee = self.cost_settings.get('fx_fee', 0.015)
                            usd_amount = action['value'] * (1 - fx_fee)
                            fx_note = f"  (환전 후 ~${usd_amount:.0f} USD)"
                        report += f"{action['ticker']} ({buy_name})\n{action['shares']}주 @${action['price']:.2f} = ${action['value']:.2f}{fx_note}  ({action['expected_pct']:+.1f}% 예상)\n"
                        report += f"⚠️ Wealthsimple 자동입금 대상을 {action['ticker']}로 변경하세요\n"
                        sell_val = 0
        ...
        try:
            if with_buttons and pending_trades:
                ...
                keyboard = [
                    ...
                    [
                        InlineKeyboardButton("💰 배당 입력", callback_data="dividend_input")
                    ]
                ]
